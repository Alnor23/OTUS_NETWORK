...
Для настройки соседства по iBGP между R14 и R15 необходимо добавить каждому маршрутизатору адрес loopback`а соответствующего соседа в конфигурацию, а также указать что обмен осуществляется между loopback интерфейсами:  
R14  
```
