...
NAT будет настроен на трансляцию адреса loopback(10.1.1.7) во внешний адрес интерфейса R15.  
Настроим трансляцию на R15: 
```
```
Для проверки выполним запрос с адреса loopback(10.1.1.7) на любой адрес снаружи AS:  
```
```
